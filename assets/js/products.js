...
const CharmData = {
  ...
  // Sold-out check. `stock` only exists once a product has synced from the
  // live backend (see refreshProductsFromAPI below) — the bundled catalog
  // has no stock field, so pieces default to "in stock" until an admin
  // sets a real quantity. 0 (or less) means sold out; undefined/null means
  // "not tracked", which we treat as always available (made-to-order).
  isSoldOut(p){
